...
static struct msm_pcm_stream_app_type_cfg
	fe_dai_app_type_cfg[MSM_FRONTEND_DAI_MM_SIZE][2][MSM_BACKEND_DAI_MAX];
...
{
	pr_info("%s: fedai_id %d, session_type %d, be_id %d, app_type %d, acdb_dev_id %d, sample_rate %d\n",
	...
	if (be_id < 0 || be_id >= MSM_BACKEND_DAI_MAX) {
		pr_err("%s: Invalid BE device id %d\n", __func__, be_id);
		return;
	}
					    ...
					    int *acdb_dev_id, int *sample_rate)
{
	...
	*sample_rate =
	...
	pr_info("%s: fedai_id %d, session_type %d, be_id %d, app_type %d, acdb_dev_id %d, sample_rate %d\n",
		__func__, fedai_id, session_type, be_id, *app_type,
...
static struct cal_block_data *msm_routing_find_topology_by_path(int path)
{
	...
	list_for_each_safe(ptr, next,
		...
		if (((struct audio_cal_info_adm_top *)cal_block->cal_info)
			->path == path) {
			pr_err("%s: found cal block for path %d\n", __func__,
			       path);
			return cal_block;
		}
	}
	pr_err("%s: Can't find topology for path %d\n", __func__, path);
	return NULL;
}
							...
							int sample_rate)
{
	...
	pr_err("%s: enter with path %d, app_type %d, acdb_id %d\n",
	...
	list_for_each_safe(ptr, next,
		...
		cal_info = (struct audio_cal_info_adm_top *)
			cal_block->cal_info;
		pr_err("%s: cal_info for current block is path %d app_type %d acdb_id %d\n",
			__func__, cal_info->path, cal_info->app_type, cal_info->acdb_id);
...
static int msm_routing_get_adm_topology(int path, int fedai_id,
					int session_type, int be_id)
{
	int topology = NULL_COPP_TOPOLOGY;
	struct cal_block_data *cal_block = NULL;
	int app_type = 0, acdb_dev_id = 0, sample_rate = 0;

	pr_info("%s: enter with path %d, fedai_id %d, session_type %d, be_id %d\n",
	       __func__, path, fedai_id, session_type, be_id);

	if (fedai_id < 0 || fedai_id > MSM_FRONTEND_DAI_MM_MAX_ID) {
		pr_err("%s: Invalid FE ID %d\n", __func__, fedai_id);
		return -EINVAL;
	} else if (session_type != SESSION_TYPE_RX &&
		   session_type != SESSION_TYPE_TX) {
	...
	/* check if get_cal_path(path) can be removed as it seems to get
	 * the same value as session_type which is passed in
	 */
	path = get_cal_path(path);
	pr_info("%s: path = %d, session_type = %d\n", __func__, path,
	       session_type);
	if (cal_data == NULL)
		goto done;
	...
	topology = ((struct audio_cal_info_adm_top *) cal_block->cal_info)
			   ->topology;
unlock:
	mutex_unlock(&cal_data->lock);
done:
	pr_info("%s: Using topology %d\n", __func__, topology);
	return topology;
}
					 ...
					 int path_type, int perf_mode)
{
	...
	if (fedai_id < 0 || fedai_id > MSM_FRONTEND_DAI_MM_MAX_ID) {
		pr_err("%s: Invalid FE ID %d\n", __func__, fedai_id);
		return;
	} else if (sess_type != SESSION_TYPE_RX &&
					  ...
					  uint32_t compr_passthr_mode)
{
	...
	pr_info("%s:fe_id[%d] perf_mode[%d] id[%d] stream_type[%d] passt[%d]",
		 __func__, fe_id, perf_mode, dspst_id, stream_type,
		 compr_passthr_mode);
	...
	for (i = 0; i < MSM_BACKEND_DAI_MAX; i++) {
		...
		if (!is_be_dai_extproc(i) &&
			...
			bit_width =
				msm_routing_get_bit_width(msm_bedais[i].format);
			app_type = fe_dai_app_type_cfg[fe_id][session_type][i]
					   .app_type;
			if (app_type) {
				app_type_idx = msm_pcm_routing_get_app_type_idx(
					app_type);
				sample_rate =
			...
			pr_info("%s: set idx bit of fe:%d, type: %d, be:%d\n",
			...
			for (j = 0; j < MAX_COPPS_PER_PORT; j++) {
				unsigned long copp =
					session_copp_map[fe_id][session_type]
							[i];
				if (test_bit(j, &copp)) {
					payload.port_id[num_copps] =
						msm_bedais[i].port_id;
		   ...
		   (test_bit(fedai_id, &msm_bedais[i].fe_sessions))) {
			...
			acdb_dev_id =
				fe_dai_app_type_cfg[fedai_id][session_type][i]
					.acdb_dev_id;
			topology = msm_routing_get_adm_topology(
				path_type, fedai_id, session_type, i);
			copp_idx = adm_open(msm_bedais[i].port_id, path_type,
					    sample_rate, channels, topology,
			...
			pr_info("%s: setting idx bit of fe:%d, type: %d, be:%d\n",
...
static void msm_pcm_routing_process_audio(u16 reg, u16 val, int set)
{
	...
	if (set) {
		...
		if (msm_bedais[reg].active && fdai->strm_id !=
			...
			app_type = fe_dai_app_type_cfg[val][session_type][reg]
					   .app_type;
			if (app_type) {
				app_type_idx =
				msm_pcm_routing_get_app_type_idx(app_type);
			...
			topology = msm_routing_get_adm_topology(
				path_type, val, session_type, reg);
			acdb_dev_id =
		...
		if(msm_bedais[be_idx].active) {
			if (afe_get_port_type(msm_bedais[be_idx].port_id) ==
				...
				if ((be_idx < MSM_BACKEND_DAI_MAX) &&
					...
					(long*)ucontrol->value.integer.value,count);
	...
	for (be_idx = 0; be_idx < MSM_BACKEND_DAI_MAX; be_idx++) {
		if(msm_bedais[be_idx].active) {
			if (afe_get_port_type(msm_bedais[be_idx].port_id) ==
				...
				if ((be_idx < MSM_BACKEND_DAI_MAX) &&
					msm_bedais[be_idx].active) {
					ret = adm_set_sound_booster_rotation(msm_bedais[be_idx].port_id,
					(long*)ucontrol->value.integer.value,count);
...
static int msm_pcm_routing_prepare(struct snd_pcm_substream *substream)
{
	...
	for_each_set_bit(i, &bedai->fe_sessions, MSM_FRONTEND_DAI_MM_SIZE) {
		...
		if (fdai->strm_id != INVALID_SESSION) {
			...
			app_type = fe_dai_app_type_cfg[i][session_type][be_id]
					   .app_type;
			if (app_type) {
				app_type_idx =
				msm_pcm_routing_get_app_type_idx(app_type);
			...
			acdb_dev_id =
				fe_dai_app_type_cfg[i][session_type][be_id]
					.acdb_dev_id;
			topology = msm_routing_get_adm_topology(
				path_type, i, session_type, be_id);
