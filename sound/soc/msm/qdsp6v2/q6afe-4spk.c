...
static struct cal_block_data *afe_find_cal(int cal_index, int port_id)
{
	...
	pr_info("%s: cal_index %d port_id %d port_index %d\n", __func__,
	...
	list_for_each_safe(ptr, next,
		...
		pr_err("%s: acdb_id %d, cal_acdb_id %d, rate %d, cal_rate %d\n",
	...
	u32 rate) /* This function is no blocking */
{
	...
	pr_info("%s: setting this afe index %d sample rate to %d\n",
		__func__, index, rate);
	this_afe.afe_sample_rates[index] = rate;

	if (this_afe.rt_cb) {
		pr_err("%s: setting this afe dev_id_tb for %d to %d\n",
			__func__, index, this_afe.rt_cb(port_id));
		...
		union afe_port_config *afe_config, int rate)
{
	...
	if ((index >= 0) && (index < AFE_MAX_PORTS)) {
		pr_err("%s: setting this afe index %d sample rate to %d\n",
			__func__, index, rate);
		this_afe.afe_sample_rates[index] = rate;
