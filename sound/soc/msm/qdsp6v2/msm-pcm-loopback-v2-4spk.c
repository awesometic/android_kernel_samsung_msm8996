					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	be_id = ucontrol->value.integer.value[3];
	pr_debug("%s: fe_id- %llu session_type- %d be_id- %d app_type- %d acdb_dev_id- %d sample_rate- %d be_id- %d\n",
		__func__, fe_id, SESSION_TYPE_RX, be_id, app_type, acdb_dev_id,
		sample_rate, be_id);
	msm_pcm_routing_reg_stream_app_type_cfg(fe_id, SESSION_TYPE_RX, be_id,
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = msm_pcm_routing_get_stream_app_type_cfg(
		fe_id, SESSION_TYPE_RX, be_id, &app_type, &acdb_dev_id,
		&sample_rate);
	if (ret < 0) {
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_debug("%s: fe_id- %llu session_type- %d be_id- %d app_type- %d acdb_dev_id- %d sample_rate- %d\n",
		__func__, fe_id, SESSION_TYPE_TX, be_id, app_type, acdb_dev_id,
		sample_rate);
	msm_pcm_routing_reg_stream_app_type_cfg(fe_id, SESSION_TYPE_TX, be_id,
						app_type, acdb_dev_id,
						sample_rate);
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = msm_pcm_routing_get_stream_app_type_cfg(
		fe_id, SESSION_TYPE_TX, be_id, &app_type, &acdb_dev_id,
		&sample_rate);
	if (ret < 0) {
		pr_err("%s: msm_pcm_routing_get_stream_app_type_cfg failed returned %d\n",
			__func__, ret);
