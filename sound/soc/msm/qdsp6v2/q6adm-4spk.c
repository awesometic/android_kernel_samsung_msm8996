	     ...
	     int perf_mode, uint16_t bit_width, int app_type, int acdb_id)
{
	...
	pr_info("%s:port %#x path:%d rate:%d mode:%d perf_mode:%d,topo_id %d, bitwidth %d, acdb %d, app_type %d\n",
...
#ifdef CONFIG_SEC_SND_SOLUTION
int adm_set_sound_booster(int port_id, long *param, int count)
{
	...
	if(count == 0)
		sb_params.params.module_id = MODULE_ID_PP_SS_SB_4SPK_2;
	else
		sb_params.params.module_id = MODULE_ID_PP_SS_SB_4SPK_1;
...
int adm_set_sound_booster_rotation(int port_id, long *param, int count)
{
	struct adm_set_pp_sb_rotation_param sb_params;
	int ret = 0;
	int port_idx;
	int copp_idx;
	...
	sb_params.params.param_id = AUDPROC_PARAM_ID_PP_SB_ROTATION_PARAM;
	sb_params.params.param_size = sb_params.command.payload_size -
					sizeof(sb_params.params);
	sb_params.params.reserved = 0;
	sb_params.sb_rotaion = param[0];

	pr_info("%s sound booster : %d\n",
		 __func__, sb_params.sb_rotaion);
	atomic_set(&this_adm.copp.stat[port_idx][copp_idx], 0);
