...
bool sec_get_param(enum sec_param_index index, void *value)
{
	...
	case param_index_qnad:
		memcpy(value, &(param_data->param_qnad_data), sizeof(struct param_qnad));
		break;
	case param_index_qnad_ddr_result:
		memcpy(value, &(param_data->param_qnad_ddr_result_data), sizeof(struct param_qnad_ddr_result));
		break;
#endif
	default:
		return false;
...
#ifdef CONFIG_SEC_NAD
	case param_index_qnad:
		memcpy(&(param_data->param_qnad_data),
				value, sizeof(struct param_qnad));
		break;
	case param_index_qnad_ddr_result:
		memcpy(&(param_data->param_qnad_ddr_result_data),
				value, sizeof(struct param_qnad_ddr_result));
		break;
#endif
	default:
		return false;
