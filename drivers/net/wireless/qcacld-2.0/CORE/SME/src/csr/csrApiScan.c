...
void csr_purge_weak_scan_results(tpAniSirGlobal mac_ctx);
                                             ...
                                             tANI_U8 sessionId)
{
                                      ...
                                      LL_ACCESS_LOCK)) != NULL)
    {
          ...
          )
        {
            ...
            csr_purge_weak_scan_results(pMac);
        }
...
/**
 * csr_purge_weak_scan_results() - This function removes weak scan entries
 * @mac_ctx: pointer to Global MAC structure
 *
 * This function removes weak scan entries
 *
 * Return: None
 */
...
void csr_purge_weak_scan_results(tpAniSirGlobal mac_ctx)
{
    ...
    tCsrScanResult *presult, *weakest_bss = NULL;
    ...
    if (weakest_bss) {
        /* Free the weakest BSS Entries */
        if(csrLLRemoveEntry(&mac_ctx->scan.scanResultList,
            &weakest_bss->Link, LL_ACCESS_NOLOCK)) {
            smsLog(mac_ctx, LOG1,
                FL("Rssi %d of BSSID to be removed" MAC_ADDRESS_STR),
                weakest_bss->Result.BssDescriptor.rssi,
                MAC_ADDR_ARRAY(
                weakest_bss->Result.BssDescriptor.bssId));
            csrFreeScanResultEntry(mac_ctx, weakest_bss);
        }
    }
