...
if [ "${REGION}" == "${CARRIER}" ]; then
	...
	VARIANT_DEFCONFIG=msm8996_sec_${MODEL}_${REGION}_${CARRIER}_defconfig
fi
...
FUNC_BUILD_KERNEL()
{
    ...
    echo '--base 0x80000000 --cmdline "console=ttyHSL0,115200,n8 androidboot.hardware=qcom msm_rtb.filter=0x237 ehci-hcd.park=3 lpm_levels.sleep_disabled=1 androidboot.selinux=permissive rcupdate.rcu_expedited=1 cma=32M@0-0xffffffff" --pagesize 4096 --tags_offset 0x02000000 --ramdisk_offset 0x02200000 --os_version 8.0.0 --os_patch_level 2018-04-01'   >  $PRODUCT_OUT/mkbootimg_ver_args.txt   
